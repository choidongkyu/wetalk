        ...
        binding.saveBtn.setOnClickListener {
            lifecycleScope.launch(Dispatchers.Main) {
                ...
                withContext(Dispatchers.Default) {
                    val result = server.setStatusMsg(Util.getSession(this@ProfileEditActivity), msg)
                    Log.d("test11", result.toString())
                    if(result == 200) { // 정상적으로 update가 되었다면
                        Util.setMyStatueMsg(msg, this@ProfileEditActivity) // 상태메시지 Pref에 저장
                    }
                }
                binding.progressbar.visibility = View.GONE
                Toast.makeText(this@ProfileEditActivity, "상태 메시지가 변경되었습니다.",Toast.LENGTH_SHORT).show()
                finish()
            }
        }
